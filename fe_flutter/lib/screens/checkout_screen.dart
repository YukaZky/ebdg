...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  String get _addressText =>
      ...
      _address?['detail_address']?.toString() ??
      '-';
  ...
  String get _signature {
    final itemKey = widget.cartItems
        .map((e) =>
            '${e['id']}:${e['product_id'] ?? e['product']?['id']}:${e['quantity']}')
        .join('|');
  ...
  Future<void> _fetchAddress() async {
    ...
    if (data is List && data.isNotEmpty) {
      try {
        mainAddress = Map<String, dynamic>.from(data.firstWhere((a) =>
            a['isdefault'] == 1 ||
            a['isdefault'] == '1' ||
            a['is_main'] == 1 ||
            a['is_main'] == '1' ||
  ...
  Future<void> _calculateShipping() async {
    if (_address == null || _address!['city_id'] == null || _courier == null)
      return;
    setState(() {
    ...
    final data = await ApiService.checkCost(
        cityId, widget.totalWeight.toInt(), _courier!);
    if (!mounted) return;
    setState(() {
  ...
  void _chooseService(String? value) {
    final selected = _shippingOptions.firstWhere(
        (e) => e['service']?.toString() == value,
        orElse: () => null);
    final costList = selected == null ? null : selected['cost'] as List?;
    setState(() {
      _service = value;
      _shippingCost = double.tryParse((costList != null && costList.isNotEmpty)
              ? costList[0]['value']?.toString() ?? '0'
              : '0') ??
          0;
      _method = null;
      _vaNumber = null;
  ...
  Future<bool> _finalizeIfReady({bool showError = false}) async {
    ...
    if (_address == null ||
      ...
      if (showError && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Lengkapi alamat dan ongkir terlebih dahulu.'),
    ...
    if (showError) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Gagal menyiapkan pesanan.'),
          backgroundColor: Colors.red));
    }
    return false;
  }
  ...
  Future<void> _selectPaymentMethod() async {
    ...
    final response = await CheckoutApiService.setPaymentMethod(
        orderId: _orderId!,
    ...
    if (response != null && response['success'] == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Gagal membuat VA/instruksi pembayaran.'),
          backgroundColor: Colors.red));
    }
  }
  ...
  String? _readVa(dynamic info, dynamic mid) {
    ...
    if (mid is Map && mid['va_numbers'] is List && mid['va_numbers'].isNotEmpty)
      return mid['va_numbers'][0]['va_number']?.toString();
    if (mid is Map && mid['permata_va_number'] != null)
      return mid['permata_va_number'].toString();
    return null;
  }
  ...
  String? _readQr(dynamic info, dynamic mid) {
    ...
    if (mid is Map && mid['actions'] is List) {
      for (final action in mid['actions']) {
        if (action is Map && action['name'] == 'generate-qr-code')
  ...
  String? _readExpiry(dynamic info, dynamic mid) {
    ...
    if (mid is Map && mid['expiry_time'] != null)
      return mid['expiry_time'].toString();
    return null;
  }
  ...
  void _setTimer(String? expiry) {
    _timer?.cancel();
    try {
      _timeLeft = expiry == null
          ? const Duration(hours: 24)
          : DateTime.parse(expiry).difference(DateTime.now());
    } catch (_) {
  ...
  Future<void> _resetPayment() async {
    ...
    if (response != null && response['success'] == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Metode pembayaran berhasil direset.')));
    }
  }
  ...
  Future<void> _checkPaymentStatus() async {
    ...
    if (response != null && response['success'] == true) {
      if (status == 'settlement' ||
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Pembayaran masih menunggu proses.')));
      }
    }
  }
  ...
  Future<void> _goToConfirmation() async {
    ...
    Navigator.pushAndRemoveUntil(
      context,
      MaterialPageRoute(
          builder: (_) => OrderConfirmationScreen(order: _order!)),
      (_) => false,
    );
  }
  ...
  void _lockedMessage() {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text(
            'Reset metode pembayaran dulu jika ingin mengubah alamat atau ongkir.')));
  }
        ...
        child: SafeArea(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              Text('Rp ${_grandTotal.toStringAsFixed(0)}',
  ...
  Widget _sectionTitle(IconData icon, String title, {String? badge}) =>
      Row(children: [
        Container(
            ...
            child: Icon(icon, size: 19, color: const Color(0xFFE65100))),
  ...
  Widget _addressCard() {
    ...
    if (_address == null) {
      return InkWell(
          ...
          child: _card(Row(children: const [
            Icon(Icons.location_off, color: Colors.red),
            SizedBox(width: 12),
            Expanded(
                child: Text('Alamat belum diatur. Klik di sini.',
                    style: TextStyle(
    ...
    return _card(
      ...
      _sectionTitle(Icons.location_on_outlined, 'Alamat Pengiriman',
          badge: 'Dipilih'),
      ...
      Container(
          ...
          child:
            ...
            Text(
                '${_address!['city_name']}, ${_address!['province_name']} - ${_address!['postal_code']}',
  ...
  Widget _shippingCard() =>
      _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _sectionTitle(Icons.local_shipping_outlined, 'Kurir Pengiriman',
            badge: _orderId == null ? null : 'Siap'),
        const SizedBox(height: 14),
        ...
        if (_loadingShipping)
          ...
          DropdownButtonFormField<String>(
              isExpanded: true,
              decoration: InputDecoration(
                  labelText: 'Pilih Layanan Ongkir',
              ...
              items: _shippingOptions.map<DropdownMenuItem<String>>((option) {
                ...
                final price = cost != null && cost.isNotEmpty
                    ? cost[0]['value']?.toString() ?? '0'
                    : '0';
                return DropdownMenuItem(
                    value: option['service']?.toString(),
                    child: Text('${option['service']} - Rp $price',
                        overflow: TextOverflow.ellipsis));
              }).toList(),
              onChanged: _paymentLocked ? null : _chooseService),
        if (_finalizing)
          const Padding(
          ...
          Padding(
              ...
              child: Row(children: const [
                ...
                Expanded(
                    child: Text(
                        'Alamat dan ongkir sudah final. Lanjut pilih metode pembayaran.',
                        style: TextStyle(
        ...
        if (_paymentLocked)
          Padding(
              ...
              child: Text(
                  'Reset metode pembayaran jika ingin mengubah alamat atau ongkir.',
                  style: TextStyle(color: Colors.grey.shade600, fontSize: 12))),
      ]));

  Widget _paymentCard(String timerText) =>
      _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _sectionTitle(
            Icons.account_balance_wallet_outlined, 'Metode Pembayaran',
            badge: _paymentState == PaymentState.pending
                ? 'Menunggu'
                : _paymentState == PaymentState.approved
                    ? 'Lunas'
                    : null),
        ...
        if (_paymentState == PaymentState.initial ||
              ...
              child: Container(
                  ...
                  child: Row(children: [
                    Container(
                        ...
                        child: _method?.iconUrl.isNotEmpty == true
                            ? Padding(
                                padding: const EdgeInsets.all(6),
                                child: Image.network(_method!.iconUrl,
                                    fit: BoxFit.contain,
                    ...
                    Expanded(
                        child: Column(
                          ...
                          Text(
                              _orderId == null
                                  ? 'Lengkapi alamat dan ongkir dulu'
                                  : (_method?.name ??
                          ...
                          Text(
                              _orderId == null
                                  ? 'VA/QRIS dibuat setelah ongkir final'
                                  : 'Tap untuk memilih bank, QRIS, atau e-wallet',
                              style: TextStyle(
          ...
          if (_paymentState == PaymentState.expired)
            const Padding(
                ...
                child: Text('Sesi pembayaran berakhir. Pilih metode kembali.',
          ...
          Container(
              ...
              child: Column(
                  ...
                  children: [
                    Row(children: [
                      Expanded(
                          child: Text(_method?.name ?? 'Metode pembayaran',
                              style: const TextStyle(
                ...
                child: Center(
                    child: Container(
                        ...
                        child: Image.network(_qrCodeUrl!,
                            width: 220, height: 200, fit: BoxFit.contain)))),
          if (_vaNumber == null && _qrCodeUrl == null)
            const Text(
                'Selesaikan pembayaran sesuai instruksi metode yang dipilih.'),
          const SizedBox(height: 14),
          Row(children: [
            Expanded(
                child: OutlinedButton.icon(
                    onPressed: _loadingPayment ? null : _checkPaymentStatus,
                    icon: const Icon(Icons.refresh),
                    label: const Text('Refresh Status'))),
            const SizedBox(width: 10),
            Expanded(
                child: OutlinedButton.icon(
                    onPressed: _loadingPayment ? null : _resetPayment,
                    icon: const Icon(Icons.restart_alt),
                    label: const Text('Reset Metode')))
          ]),
        ],
        if (_paymentState == PaymentState.approved)
          const Center(
  ...
  Widget _summaryCard() =>
      _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _sectionTitle(Icons.receipt_long_outlined, 'Ringkasan Pesanan'),
        const SizedBox(height: 14),
  ...
  Widget _bottomButton() {
    ...
    return ElevatedButton(
      onPressed: _orderId == null || _loadingPayment || _finalizing
          ? null
      ...
      child: Text(
        _finalizing ? 'MENYIAPKAN PESANAN...' : 'PILIH METODE PEMBAYARAN',
        style:

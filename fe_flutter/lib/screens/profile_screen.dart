...
class _ProfileScreenState extends State<ProfileScreen> {
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  _buildOrderCard(isLoggedIn),
                  const SizedBox(height: 14),
                  _buildAccountServices(isLoggedIn),
                  const SizedBox(height: 14),
                  _buildStoreCard(isLoggedIn),
          ...
          child: Column(children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              Row(children: [
                ...
                _circleAction(Icons.logout_rounded, isLoggedIn ? _handleLogout : null),
              ]),
            ]),
            const SizedBox(height: 20),
            Container(
                  ...
                  child: Stack(clipBehavior: Clip.none, children: [
                    Container(
                      ...
                      child: avatarUrl.isNotEmpty
                          ? Image.network(avatarUrl, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.person_rounded, size: 48, color: _primary))
  ...
  Widget _miniAuthButton(String text, Color bg, Color fg, VoidCallback onTap) {
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: onTap,
        style: ElevatedButton.styleFrom(backgroundColor: bg, foregroundColor: fg, elevation: 0, padding: const EdgeInsets.symmetric(horizontal: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
        child: Text(text, style: const TextStyle(fontWeight: FontWeight.w900)),
      ),
    );
  }

  Widget _buildOrderCard(bool isLoggedIn) {
    return _card(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Pesanan Saya', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: Colors.black87)),
          TextButton(onPressed: () => _handleFeatureTap(isLoggedIn, () {}), child: const Text('Lihat Semua')),
        ]),
  ...
  Widget _buildAccountServices(bool isLoggedIn) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _sectionHeader('Layanan Akun'),
      const SizedBox(height: 10),
      Row(children: [
        Expanded(child: _serviceTile('Alamat Saya', 'Kelola alamat pengiriman', Icons.location_on_rounded, () => _handleFeatureTap(isLoggedIn, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AddressListScreen()))))),
        const SizedBox(width: 12),
        Expanded(child: _serviceTile('Kupon Saya', 'Voucher dan promo', Icons.local_activity_rounded, () => _handleFeatureTap(isLoggedIn, () {}))),
      ]),
    ]);
  }
  ...
  Widget _buildStoreCard(bool isLoggedIn) {
    return InkWell(
      onTap: () => _handleFeatureTap(isLoggedIn, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AdminDashboardScreen()))),
      borderRadius: BorderRadius.circular(22),
      child: Container(
        padding: const EdgeInsets.all(18),
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: [_primary, Color(0xFF174C7E)], begin: Alignment.topLeft, end: Alignment.bottomRight),
          borderRadius: BorderRadius.circular(22),
          boxShadow: [BoxShadow(color: _primary.withOpacity(0.18), blurRadius: 18, offset: const Offset(0, 8))],
        ),
        child: Row(children: [
  ...
  Widget _card({required Widget child, EdgeInsetsGeometry padding = const EdgeInsets.all(16)}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: _primary.withOpacity(0.04), blurRadius: 16, offset: const Offset(0, 8))],
      ),
      child: child,
    );
  }
  ...
  Widget _sectionHeader(String title) {
    return Row(children: [
      Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: Colors.black87)),
    ...
    return FutureBuilder<List<Product>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return _card(child: Text('Produk rekomendasi belum bisa dimuat.', style: TextStyle(color: Colors.grey.shade700)));
        }
        final products = (snapshot.data ?? <Product>[]).take(4).toList();
        if (products.isEmpty) {
          return _card(child: Text('Belum ada produk tersedia.', style: TextStyle(color: Colors.grey.shade700)));
        }

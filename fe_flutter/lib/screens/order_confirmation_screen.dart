...
class _OrderConfirmationScreenState extends State<OrderConfirmationScreen> {
  ...
  String _statusKey() {
    ...
    final trxStatus =
        _map(order['transaction'])['status']?.toString().toLowerCase() ?? '';
  ...
  Color get _gradientEnd =>
      _isCanceled ? _dangerDark : const Color(0xFF123A68);
  ...
  bool get _canReview =>
      !_isCanceled &&
      (_statusKey() == 'delivered' || _statusKey() == 'done');
  ...
  void _handlePrimaryAction(BuildContext context) {
    if (_canResumeCheckout) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => ResumeOrderCheckoutScreen(order: order),
        ),
      );
      return;
    }
  ...
  String _receiptText() {
    ...
    final paymentId =
        paymentInfo['transaction_id'] ?? transaction['payment_token'] ?? '-';
    ...
    final buffer = StringBuffer()
      ...
      ..writeln(
        'Metode: ${(details['payment_type'] ?? paymentInfo['payment_type'] ?? '-').toString().toUpperCase()}',
      )
      ...
      ..writeln(
        'Kurir: ${order['mode_pengiriman'] ?? '-'} - ${order['jenis_pengiriman'] ?? '-'}',
      )
      ..writeln('')
      ..writeln('BARANG');
    ...
    for (final raw in _list(order['items'])) {
      ...
      buffer.writeln(
        '$qty x ${product['name'] ?? 'Produk'} = ${_money(price * qty)}',
      );
    }
        ...
        builder: (context, controller) => Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Icon(Icons.print_rounded, color: _main),
                  const SizedBox(width: 10),
                  Text(
                    'Preview Resi Pesanan',
                    style: TextStyle(
                      fontSize: 16,
                      color: _main,
                ...
                child: ElevatedButton.icon(
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Detail resi berhasil disalin.'),
                      ),
                    );
                  },
  ...
  Future<void> _showReviewSheet() async {
    ...
    if (products.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Produk pada pesanan ini belum tersedia untuk diulas.'),
        ),
      );
      return;
    }
    ...
    final available = products
        .where((p) => !_reviewedProductIds.contains(p['product_id']))
        .toList();

    if (available.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Semua produk pada pesanan ini sudah diberi penilaian.'),
        ),
      );
      return;
    }
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Ulasan',
                      hintText: 'Tulis pengalaman kamu tentang produk ini',
                      border: OutlineInputBorder(),
                    ),
                  ),
                    ...
                    child: ElevatedButton.icon(
                      onPressed: _submittingReview
                          ...
                          : () async {
                              ...
                              final ok = await MarketplaceApiService.addReview(
                              ...
                              if (ok) {
                                setState(() {
                                  _reviewedProductIds.add(selectedProductId);
                                });
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Gagal menyimpan penilaian.'),
                                  ),
                                );
                              }
                            },
              ...
              child: Column(
                ...
                children: [
                  ...
                  _card([
                    ...
                    _row(
                      'Bank',
                      (details['bank'] ?? '-').toString().toUpperCase(),
                    ),
                    if (vaNumber.toString() != '-' && !_isCanceled)
                      _row('Nomor VA', vaNumber.toString()),
                  ]),
                  const SizedBox(height: 14),
                  _title('Pengiriman'),
                  _card([
                    ...
                    _row(
                      'Kurir',
                      '${order['mode_pengiriman'] ?? '-'} - ${order['jenis_pengiriman'] ?? '-'}',
                    ),
                    Padding(
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            _isCanceled
                                ? 'Order #${order['id'] ?? '-'} sudah dibatalkan'
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(_statusIcon(), color: _main, size: 22),
          ),
                ...
                Text(
                  _isCanceled
                      ...
                      : _canReview
                          ...
                          : _canResumeCheckout
                              ? 'Pesanan ini masih bisa dilanjutkan dari halaman detail.'
                              : 'Detail akhir pesanan sudah dapat dilihat di halaman ini.',
                  style: const TextStyle(
                    fontSize: 11.5,
                    color: _muted,
                    height: 1.35,
  ...
  Widget _productRow(dynamic value) {
    ...
    final reviewed = _reviewedProductIds.contains(productId);
      ...
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            ...
            color: strong ? _main : const Color(0xFF111827),
            fontWeight: FontWeight.w900,
          ),
        ),
      ],

...
class _ResumeOrderCheckoutScreenState extends State<ResumeOrderCheckoutScreen> {
  ...
  void initState() {
    ...
    timer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (!mounted) return;
      setState(() { if (left.inSeconds > 0) left -= const Duration(seconds: 1); });
    });
  }
  ...
  Future<void> refreshStatus() async {
    final id = order['id']?.toString(); if (id == null) return;
  ...
  Future<void> cancelOrder() async {
    final id = order['id']?.toString(); if (id == null || loading) return;
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    appBar: AppBar(title: const Text('Lanjutkan Checkout', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w800)), backgroundColor: navy, iconTheme: const IconThemeData(color: Colors.white)),
    body: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(16, 16, 16, 130), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [paymentCard(), section('Daftar Barang'), productCard(), section('Total Order'), totalCard()])),
    bottomSheet: Container(padding: const EdgeInsets.fromLTRB(16, 12, 16, 16), decoration: BoxDecoration(color: Colors.white, boxShadow: [BoxShadow(color: Colors.black.withOpacity(.08), blurRadius: 16, offset: const Offset(0, -5))]), child: SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
      SizedBox(width: double.infinity, child: paid ? ElevatedButton(onPressed: loading ? null : completeCheckout, style: btn(), child: const Text('SELESAIKAN CHECKOUT', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w800))) : OutlinedButton.icon(onPressed: loading ? null : refreshStatus, icon: const Icon(Icons.refresh_rounded), label: const Text('REFRESH STATUS'), style: OutlinedButton.styleFrom(foregroundColor: navy, side: const BorderSide(color: navy), padding: const EdgeInsets.symmetric(vertical: 13), textStyle: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800)))),
      const SizedBox(height: 8),
      SizedBox(width: double.infinity, child: TextButton.icon(onPressed: loading ? null : cancelOrder, icon: const Icon(Icons.cancel_outlined, size: 18), label: const Text('BATALKAN PESANAN', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w900)), style: TextButton.styleFrom(foregroundColor: danger))),
    ]))),
  );

  ButtonStyle btn() => ElevatedButton.styleFrom(backgroundColor: navy, padding: const EdgeInsets.symmetric(vertical: 13), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)));
  Widget section(String t) => Padding(padding: const EdgeInsets.fromLTRB(0, 14, 0, 8), child: Text(t, style: const TextStyle(fontSize: 13, color: navy, fontWeight: FontWeight.w900)));
  BoxDecoration box() => BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFE2E8F0)));

  Widget paymentCard() => Container(width: double.infinity, padding: const EdgeInsets.all(16), decoration: BoxDecoration(gradient: const LinearGradient(colors: [navy, Color(0xFF163A73)]), borderRadius: BorderRadius.circular(18)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(paid ? 'PEMBAYARAN DITERIMA' : 'MENUNGGU PEMBAYARAN', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w900)), const SizedBox(height: 12),
    Text(timerText, style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)), const SizedBox(height: 14),
    Container(width: double.infinity, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      if (va != null) ...[const Text('Nomor Virtual Account', style: TextStyle(fontSize: 11, color: Color(0xFF64748B), fontWeight: FontWeight.w700)), Row(children: [Expanded(child: SelectableText(va!, style: const TextStyle(fontSize: 18, color: navy, fontWeight: FontWeight.w900))), IconButton(onPressed: copyVa, icon: const Icon(Icons.copy_rounded, color: navy))])],
      if (qr != null) Center(child: Image.network(qr!, width: 190, height: 190, fit: BoxFit.contain)),
      if (va == null && qr == null) const Text('Detail pembayaran belum tersedia. Tekan refresh status atau batalkan pesanan.', style: TextStyle(fontSize: 12, color: Color(0xFF475569))),
    ])),
  ]));

  Widget productCard() => Container(width: double.infinity, padding: const EdgeInsets.all(14), decoration: box(), child: Column(children: items().map((raw) { final item = map(raw); final product = map(item['product']); final qty = numOf(item['quantity']); final price = numOf(item['price']); return Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [Expanded(child: Text('${qty.toInt()}x ${product['name'] ?? 'Produk'}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700))), Text(money(price * qty), style: const TextStyle(fontSize: 12, color: navy, fontWeight: FontWeight.w900))])); }).toList()));
  ...
  Widget row(String l, String v, {bool strong = false}) => Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(l, style: TextStyle(fontSize: strong ? 14 : 12, color: strong ? navy : const Color(0xFF64748B), fontWeight: strong ? FontWeight.w900 : FontWeight.w500)), Text(v, style: TextStyle(fontSize: strong ? 18 : 12.5, color: strong ? navy : const Color(0xFF111827), fontWeight: FontWeight.w900))]);
}

...
class _OrderHistoryScreenState extends State<OrderHistoryScreen> {
  ...
  String status(Map<String, dynamic> order) {
    ...
    if (raw == 'packing' || raw == 'processing' || raw == 'shipped') {
      return 'packing';
    }

    if (trx == 'approved' || trx == 'settlement' || trx == 'capture') {
  ...
  String timerText(Map<String, dynamic> order) {
    ...
    try {
      ...
      return '${left.inHours.toString().padLeft(2, '0')}:'
          '${left.inMinutes.remainder(60).toString().padLeft(2, '0')}:'
  ...
  List<dynamic> filtered(List<dynamic> orders) {
    if (_tab == 'all') return orders;

    return orders.where((e) => status(map(e)) == _tab).toList();
  }

  int countBy(List<dynamic> orders, String key) {
    if (key == 'all') return orders.length;

    return orders.where((e) => status(map(e)) == key).length;
  }

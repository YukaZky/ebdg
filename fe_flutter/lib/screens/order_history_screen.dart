...
class _OrderHistoryScreenState extends State<OrderHistoryScreen> {
  ...
  String status(Map<String, dynamic> order) {
    ...
    if (raw == 'done' ||
        raw == 'completed' ||
        raw == 'complete' ||
        raw == 'selesai') return 'done';
    if (raw == 'delivered' || raw == 'deliver') return 'delivered';
    if (raw == 'packing' ||
        raw == 'processing' ||
        raw == 'shipped' ||
        raw == 'dikemas') return 'packing';
    if (raw == 'paid' ||
        raw == 'dibayar' ||
        trx == 'approved' ||
        trx == 'settlement' ||
        trx == 'capture') return 'paid_not_checked_out';
  ...
  String label(Map<String, dynamic> order) {
    ...
    return tabs.firstWhere((t) => t[0] == status(order),
        orElse: () => [
              'pending_payment',
              'Belum Dibayar',
  ...
  String timerText(Map<String, dynamic> order) {
    ...
    final expiry =
        (order['payment_deadline'] ?? info['expiry_time'])?.toString();
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(product['name']?.toString() ?? '${items.length} item produk',
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              style:
          ...
          if (s == 'pending_payment')
            Padding(
                ...
                child: Text('Sisa waktu pembayaran: ${timerText(order)}',
                    style: const TextStyle(
                        fontSize: 11.5,
          ...
          if (s == 'canceled')
            const Padding(
                padding: EdgeInsets.only(top: 8),
                child: Text('Pesanan ini sudah dibatalkan.',
                    style: TextStyle(
                        fontSize: 11.5,
                        color: danger,
                        fontWeight: FontWeight.w800))),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text(money(order['total']),
                style: TextStyle(
                    fontSize: 15, color: color, fontWeight: FontWeight.w900))
          ]),
        ]),
      ),
    );

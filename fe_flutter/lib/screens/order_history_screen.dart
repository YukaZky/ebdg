...
class OrderHistoryScreen extends StatefulWidget {
  ...
  const OrderHistoryScreen({Key? key, this.initialStatus = 'all'})
    : super(key: key);
...
class _OrderHistoryScreenState extends State<OrderHistoryScreen> {
  ...
  final List<List<dynamic>> tabs = const [
    ...
    ['proof_submitted', 'Verifikasi', Icons.fact_check_outlined],
    ['proof_rejected', 'Bukti Ditolak', Icons.highlight_off_outlined],
  ...
  String status(Map<String, dynamic> order) {
    ...
    final trx =
        map(order['transaction'])['status']?.toString().toLowerCase() ??
        order['transaction_status']?.toString().toLowerCase() ??
        '';
  ...
  String timerText(Map<String, dynamic> order) {
    ...
    final expiry = (order['payment_deadline'] ?? info['expiry_time'])
        ?.toString();
  ...
  List<dynamic> filtered(List<dynamic> orders) => _tab == 'all'
      ? orders
      : orders.where((e) => status(map(e)) == _tab).toList();
  int countBy(List<dynamic> orders, String key) => key == 'all'
      ? orders.length
      : orders.where((e) => status(map(e)) == key).length;
  ...
  Color statusColor(String s) =>
      s == 'canceled' || s == 'proof_rejected' ? danger : navy;
                  ...
                  child: Builder(
                    builder: (context) {
                      ...
                      if (snap.hasError) {
                        ...
                        return Center(
                          child: Text(
                            message.isEmpty ? 'Belum login' : message,
                          ),
                        );
                      }
                      ...
                      if (data.isEmpty)
                        return ListView(
                          ...
                          children: const [
                            SizedBox(height: 220),
                            Center(
                              child: Text('Belum ada pesanan pada status ini.'),
                            ),
                          ],
                        );
                      return ListView.builder(
                        ...
                        itemBuilder: (_, i) => card(map(data[i])),
                      );
                    },
                  ),
                ),
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          const Spacer(),
                          _countBadge(count, active, activeColor),
                        ],
                      ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              product['name']?.toString() ?? '${items.length} item produk',
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              style: const TextStyle(
                fontSize: 12.5,
            ...
            Text(
              '${items.length} item • ${order['mode_pengiriman'] ?? '-'} ${order['jenis_pengiriman'] ?? ''}',
            ...
            if (s == 'pending_payment')
              Padding(
                ...
                child: Text(
                  'Sisa waktu pembayaran: ${timerText(order)}',
                  style: const TextStyle(
                    fontSize: 11.5,
            ...
            if (s == 'paid_not_checked_out')
              const Padding(
                padding: EdgeInsets.only(top: 8),
                child: Text(
                  'Pesanan sudah dibayar dan menunggu toko memproses menjadi dikemas.',
                  style: TextStyle(
                    fontSize: 11.5,
            ...
            if (s == 'done')
              const Padding(
                padding: EdgeInsets.only(top: 8),
                child: Text(
                  'Pesanan selesai. Buka detail untuk memberi penilaian.',
                  style: TextStyle(
                    fontSize: 11.5,
                    color: Color(0xFF15803D),
                    fontWeight: FontWeight.w800,
